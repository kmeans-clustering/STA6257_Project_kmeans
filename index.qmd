...
melt(customer) %>% ggplot(aes(x=variable, y=value)) + geom_boxplot(aes(fill=variable), outlier.colour = NULL) + theme(legend.position="none")
```
...
k.max <- 15
wss <- sapply(1:k.max, 
              function(k){kmeans(customer_scaled, k, nstart=50,iter.max = 15 )$tot.withinss})
# wss
plot(1:k.max, wss,
...
set.seed(123)
k = 3
m1.kmean <- customer_scaled %>% kmeans(k, iter.max = 200, nstart = 25)
...
fviz_cluster(m1.kmean, data = customer[, -4],
...
fviz_cluster(m1.kmean, data = customer,choose.vars = c("LastSeen","Amount"),
             palette = c("#2E9FDF", "#00AFBB", "#E7B800"), 
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw()
...
s3d <- scatterplot3d(customer[,1:3], pch = 16, color=colors)
legend("right",legend = paste("Cluster", 1:3),
...
# melted<- melt(customer[c(2:4, 5)],id.vars="Cluster")
#  ggplot(melted, aes(x = Cluster, y = value)) + 
#      geom_boxplot()+facet_wrap(~variable)


customer %>% ggplot(aes(x=Cluster, y=Amount, color=Cluster)) + geom_boxplot() + facet_wrap(~Cluster)

customer %>% ggplot(aes(x=Cluster, y=Frequency, color=Cluster)) + geom_boxplot() + facet_wrap(~Cluster)

customer %>% ggplot(aes(x=Cluster, y=LastSeen, color=Cluster)) + geom_boxplot() + facet_wrap(~Cluster)
```
### Analysis
